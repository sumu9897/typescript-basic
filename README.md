...
- inferred Typing
  ...
  ```js
  let userName ='sumu'; // data type inferred as string
  ```
...
2. **object**: Represents any non-primitive value.
   ...
   user1 = { userName: 'sumu', userId: 101 };
   users.push(user1);

   let user2: { userName: string; userId: number };
   user2 = { userName: 'rana', userId: 102 };
...
3. **array**: Represents an array of values of a specific type. 2 ways to declare: `number[]` or `Array<number>`
   ...
   // let users = ["sumu", "rana", "maha"];

   // let users: string[];
   // users = ["sumu", "rana", "maha"];
...
8. **class Type OOP** - You can create class type as well. class can have constructor, properties, methods. create object  let objectName = new className()
    ...
    let user1 = new User('Mohammad Sumon', 24);
    user1.display();
...
10. **Interface vs type**
    ...
    const s1 = new Student('mohammad sumon', '1531529480');
...
- Function signature
  ...
  userInfo1 = () => {
    console.log('Mohammad Sumon is 24 years old');
  };
  ...
  userInfo1();
  userInfo2('Mohammad Sumon');
  console.log(userInfo3('Mohammad Sumon'));
  ```
